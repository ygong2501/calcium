...
class DatasetPanel:
    """Panel for dataset creation and management."""
    ...
    def __init__(self, parent, main_window):
        ...
        # Create main frame
        self.frame = ttk.LabelFrame(parent, text="Dataset Creation")
    ...
    def _process_dataset(self, source_dir, output_dir, dataset_name, split_ratios):
        """
        Process dataset creation in a background thread.
        ...
        try:
            ...
            # Check if source directory has a batch structure or direct image/label structure
            ...
            if os.path.isdir(img_dir) and os.path.isdir(label_dir):
                # Direct structure - run split_existing_dataset
                self.main_window.root.after(0, lambda: self.status_var.set("Splitting dataset..."))
                result = dataset_manager.split_existing_dataset(source_dir, split_ratios)
                dataset_path = result['dataset_info']['name']
            else:
                # Batch structure - need to gather simulations
                self.main_window.root.after(0, lambda: self.status_var.set("Creating dataset framework..."))
                dataset_info = dataset_manager.create_dataset(name=dataset_name, split_ratios=split_ratios)
                ...
                for item in os.listdir(source_dir):
                    ...
                    if os.path.isdir(item_path):
                        # Check if it has images/labels subdirectories
                        if os.path.isdir(os.path.join(item_path, 'images')) and \
                           os.path.isdir(os.path.join(item_path, 'labels')):
                            sim_dirs.append(item_path)
                
                # Process each simulation directory
                for i, sim_dir in enumerate(sim_dirs):
                    self.main_window.root.after(0, lambda: self.progress_var.set((i+1) / len(sim_dirs) * 100))
                    self.main_window.root.after(0, lambda: self.status_var.set(f"Processing simulation {i+1}/{len(sim_dirs)}..."))
                ...
                dataset_manager.generate_dataset_stats()
                dataset_path = os.path.join(output_dir, dataset_name)
            
            # Show success message
            self.main_window.root.after(0, lambda: self.status_var.set("Dataset creation complete"))
            ...
            self.main_window.root.after(0, lambda: messagebox.showerror("Dataset Error", error_msg))
